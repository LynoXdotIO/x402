...
describe("withPaymentInterceptor()", () => {
  ...
  beforeEach(async () => {
    ...
    const { selectPaymentRequirements } = await import("@lynoxdotio/x402/client");
  ...
  it("should reject if payment header creation fails", async () => {
    ...
    const { createPaymentHeader } = await import("@lynoxdotio/x402/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("@lynoxdotio/x402/types", async () => {
      const actual = await vi.importActual("@lynoxdotio/x402/types");
      return {
        ...actual,
    ...
    vi.doMock("@lynoxdotio/x402/client", () => ({
